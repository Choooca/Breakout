...
Paddle::Paddle(int screen_width, int screen_height) {
	m_position_x = screen_width * 0.5f;
	m_position_y = screen_height * 0.9f;
}
...
void Paddle::Input(const std::unique_ptr<InputHandler>& input_handler, const std::unique_ptr<Window>& window) {
	...
	m_position_x = std::clamp(m_position_x, 0.0f + m_size_x * .5f, window->GetWidth() - m_size_x * .5f);
...
void Paddle::Render(const std::unique_ptr<Window>& window) {
	...
	SDL_FRect paddle_rect = { m_position_x - m_size_x * .5f, m_position_y, m_size_x, m_size_y };
